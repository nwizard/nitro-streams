...
class Main:
    ...
    def __init__(self):
        ...
        common.log('Nitro Streams initialized')
    ...
    def downloadVideo(self, url, title):
        ...
        title = getKeyboard(default = fu.cleanFilename(title),heading='Nitro Streams')
    ...
    def parseView(self, lItem):
        ...
        if url == common.Paths.favouritesFile or url.startswith('favfolders'):
    ...
    def downloadCustomModule(self):
        ...
        if success == True:            
            # refresh container if Nitro Streams is active
            currContainer = xbmcUtils.getContainerFolderPath()
            common.showNotification('Nitro Streams', 'Download successful', 1000)
            if currContainer.startswith(self.base):                
                xbmc.executebuiltin('Container.Refresh()')
            return True
        elif success == False:        
            common.showNotification('Nitro Streams', 'Download failed', 1000)
        return False
        ...
        if definedIn:
            ...
            if definedIn.endswith('favourites.cfg') or definedIn.startswith("favfolders/"):
                ...
                if lItem['title'] != "Favourites":
                        # Add to favourites
                        contextMenuItem = createContextMenuItem('Add to Nitro Streams favourites', Mode.ADDTOFAVOURITES, codedItem)
                        contextMenuItems.append(contextMenuItem)
    ...
    def update(self):
        ...
        def checkForUpdates():
            updates = {}          
            common.showNotification('Nitro Streams', common.translate(30275))
        ...
        def doUpdates(typeName, updates):
            ...
            head = "Nitro Streams Updates - %s" % typeName
        ...
        if count == 0:
            common.showNotification('Nitro Streams', common.translate(30273))
            return
        else:
    ...
    def queueAllVideos(self, item):
        dia = DialogProgress()
        dia.create('Nitro Streams', 'Get videos...' + item['title'])
        dia.update(0)
    ...
    def executeItem(self, item):
        ...
        if '(' in url:
            ...
            if xbmcCommand.lower() in ['activatewindow', 'runscript', 'runplugin', 'playmedia', 'runaddon']:
    ...
    def run(self, argv=None):
        ...
        self.addon = Addon('plugin.video.nitro.streams', argv)

        common.log('Nitro Streams running')
        ...
        try:
            ...
            common.showError('Error running Nitro Streams')
            common.log('Error running Nitro Streams. Reason:' + str(e))
